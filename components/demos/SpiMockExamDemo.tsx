...
const SpiMockExamDemo: React.FC<SpiMockExamDemoProps> = ({ onNavigate }) => {
    const { userProfile, setSpiMockExamScore, markModuleAsCompleted, awardAchievement, updateExamState, updateCachedReport, handleApiError } = useUser();
    ...
    const [aiReview, setAiReview] = useState<AIStudyPlan | null>(userProfile?.cachedExamReport || null);
    const [isReviewLoading, setIsReviewLoading] = useState(false);
    const [reviewError, setReviewError] = useState<string | null>(null);

    // Persist local state to profile periodically
    useEffect(() => {
    ...
    }, [examState, questions, currentQuestionIndex, userAnswers, flaggedQuestions, timeLeft, updateExamState]);

    const startExam = (isResume = false) => {
        if (isResume && userProfile?.currentExamState) {
            const saved = userProfile.currentExamState;
            ...
            setTimeLeft(2 * 60 * 60); 
            updateExamState(null); // Clear previous if starting fresh
        }
    ...
    const finishExam = useCallback(() => {
        ...
        updateExamState(null); // Clear active session on finish
    }, [questions, userAnswers, setSpiMockExamScore, markModuleAsCompleted, awardAchievement, updateExamState]);
    
    const handleGetAiReview = async () => {
        if (aiReview) return; // Prevent double trigger
        ...
        if (incorrectAnswers.length === 0) {
            ...
            updateCachedReport(perfectScoreReview);
        ...
        Wrong Questions: ${JSON.stringify(incorrectAnswers.map(q => ({ question: q.question, userAnswer: q.userAnswer, correctAnswer: q.correctAnswer, explanation: q.explanation })))}`;
        ...
        try {
            ...
            const parsedReview: AIStudyPlan = JSON.parse(response.text);
            addHistoryItem({ type: 'examReview', content: parsedReview, context: 'SPI Mock Exam Review' });
            setAiReview(parsedReview);
            updateCachedReport(parsedReview);
        } catch (e) {
            handleApiError(e);
            setReviewError("System latency high. Could not generate neural review. Please try again later.");
        } finally {
            setIsReviewLoading(false);
        }
    };
    ...
    if (examState === 'start') {
        const canResume = !!userProfile?.currentExamState;
        return (
            <div className="flex flex-col items-center justify-center h-full text-center p-8 bg-white/5 rounded-2xl">
                ...
                <div className="flex gap-4">
                    {canResume && <ControlButton onClick={() => startExam(true)}>Resume Session</ControlButton>}
                    <ControlButton onClick={() => startExam(false)} secondary={canResume}>
                        {canResume ? 'Start Fresh' : 'Begin Exam'}
                    </ControlButton>
                </div>
            </div>
        );
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full text-center p-4 sm:p-8 bg-white/5 rounded-2xl overflow-y-auto">
                ...
                <div className="flex gap-4 flex-wrap justify-center">
                    ...
                    <ControlButton onClick={() => startExam(false)} secondary>Take Again</ControlButton>
                </div>

                <div className="mt-8 w-full flex flex-col items-center">
                    <ControlButton onClick={handleGetAiReview} disabled={isReviewLoading || !!aiReview}>
                        {isReviewLoading ? "Analyzing..." : aiReview ? "Review Saved Below" : "Get AI-Powered Review ✨"}
                    </ControlButton>
            ...
            {examState === 'paused' && (
                 <div className="absolute inset-0 bg-black/80 z-20 flex flex-col justify-center items-center gap-6">
                    ...
                    <div className="flex gap-4">
                        ...
                        <ControlButton onClick={() => {if(window.confirm('End exam and calculate current score?')) finishExam()}} secondary>End Exam</ControlButton>
                    </div>
                </div>
            )}
            ...
            <div className="flex-shrink-0 flex justify-between items-center p-4 bg-gray-900/50 rounded-b-2xl border-t border-white/10">
                ...
                <div className="flex gap-2">
                    ...
                    {viewMode === 'exam' && currentQuestionIndex === questions.length - 1 ? (
                        <ControlButton onClick={() => {if(window.confirm('Finish exam?')) finishExam()}}>
                            Finish Exam
                        </ControlButton>
                    ) : (
