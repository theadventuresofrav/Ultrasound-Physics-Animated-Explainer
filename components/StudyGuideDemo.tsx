...
const TopicBriefingButton: React.FC<{ title: string, content: string }> = ({ title, content }) => {
    const { isBriefingActive, playBriefing, stopBriefing } = useSound();
    ...
    const handleTopicAudio = async (e: React.MouseEvent) => {
        e.stopPropagation();
        if (localIsNarrating) {
            stopBriefing();
            return;
        ...
        try {
            ...
            if (cachedAudio) {
                setStatus("PLAYING...");
                // Fixed: playBriefing only accepts 1 argument. Await it as it returns a promise that resolves on end.
            ...
            const response = await ai.models.generateContent({
                ...
                contents: [{ parts: [{ text: `Tactical briefing for ${title}. ${content}` }] }],
                config: {
    ...
    return (
        <button
            onClick={handleTopicAudio}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all ${localIsNarrating
        ...
        >
            <BriefingIcon isActive={localIsNarrating} />
                ...
                {isNodeOpen && (
                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                        <div className="p-4 pt-0 space-y-4 border-t border-white/5">
                            <div className="text-sm text-white/60 leading-relaxed py-2 whitespace-pre-wrap">
                                {topic.content}
                            </div>
...
const IntelligenceArchive: React.FC = () => {
    ...
    const handleNarrate = async () => {
        ...
        try {
            const sanitizeKey = (str: string) => str.replace(/[^a-zA-Z0-9]/g, '_');
            const cacheKey = `${NARRATION_CACHE_KEY_PREFIX}_${sanitizeKey(selectedTerm.term)}`;
            const cachedAudio = localStorage.getItem(cacheKey);
            ...
            const response = await ai.models.generateContent({
                model: "gemini-2.5-flash-preview-tts",
                contents: [{ parts: [{ text: `Tactical briefing for ${selectedTerm.term}. ${selectedTerm.definition}` }] }],
                config: {
                    responseModalities: [Modality.AUDIO],
                    speechConfig: { voiceConfig: { prebuiltVoiceConfig: { voiceName: 'Charon' } } },
                },
                        ...
                        >
                            <header className="mb-8 flex justify-between items-start">
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${localIsNarrating ? 'bg-red-500/20 border-red-500 text-red-400' : 'bg-white/5 border-white/10 text-white/50 hover:bg-white/10 hover:text-white hover:border-[var(--gold)]/40'}`}
                                >
                                    <BriefingIcon isActive={localIsNarrating} />
                                    <span>{audioStatus || 'Tactical Audio'}</span>
                                </button>
                            </header>
                ...
                <div className="flex items-center gap-1 w-full sm:w-auto overflow-x-auto no-scrollbar">
                    {(['Archives', 'Database', 'Training', 'Assessment'] as Tab[]).map(tab => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap ${activeTab === tab
